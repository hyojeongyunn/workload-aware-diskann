...
// Query-aware partition function that uses sampled queries for training
template<typename T>
int partition_with_ram_budget_query_aware(const std::string data_file,
  ...
  // Check number of sampled queries and sample if needed
  size_t num_sampled_queries, query_dim;
  diskann::get_bin_metadata(sampled_query_file, num_sampled_queries, query_dim);
  
  const size_t MAX_KMEANS_TRAINING_POINTS = 8388608; // k-means++ limit
  double query_sampling_rate = 1.0;
  
  if (num_sampled_queries > MAX_KMEANS_TRAINING_POINTS) {
    query_sampling_rate = (double)MAX_KMEANS_TRAINING_POINTS / (double)num_sampled_queries;
    diskann::cout << "Sampled queries (" << num_sampled_queries 
                  << ") exceed k-means limit. Sampling with rate: " 
                  << query_sampling_rate << std::endl;
  } else {
    diskann::cout << "Using all " << num_sampled_queries 
                  << " sampled queries for training" << std::endl;
  }
  
  // Sample from sampled query file for training
  gen_random_slice<T>(sampled_query_file, query_sampling_rate, train_data_float, 
                      num_train, train_dim);

  // Generate test data by sampling from base file for cluster size estimation
  const double sampling_rate = 0.01; // small sample for testing
  size_t test_dim;
  size_t num_test;
  float *test_data_float;
  gen_random_slice<T>(data_file, sampling_rate, test_data_float, num_test,
  ...
  while (!fit_in_ram) {
    ...
    diskann::cout << "Processing global k-means with " << num_parts << " parts (query-aware)"
                  << std::endl;
    kmeans::kmeanspp_selecting_pivots(train_data_float, num_train, train_dim,
                                      pivot_data, num_parts);
  ...
  diskann::cout << "Saving global k-center pivots (query-aware)" << std::endl;
  diskann::save_bin<float>(output_file.c_str(), pivot_data, (size_t) num_parts,
                           train_dim);
...
template DISKANN_DLLEXPORT int partition_with_ram_budget_query_aware<int8_t>(
    const std::string data_file, const std::string sampled_query_file, 
    double ram_budget, size_t graph_degree, const std::string prefix_path, size_t k_base);
template DISKANN_DLLEXPORT int partition_with_ram_budget_query_aware<uint8_t>(
    const std::string data_file, const std::string sampled_query_file, 
    double ram_budget, size_t graph_degree, const std::string prefix_path, size_t k_base);
template DISKANN_DLLEXPORT int partition_with_ram_budget_query_aware<float>(
    const std::string data_file, const std::string sampled_query_file, 
    double ram_budget, size_t graph_degree, const std::string prefix_path, size_t k_base);