...
int main(int argc, char** argv) {
  std::string data_type, dist_fn, data_path, sampled_query_path, index_path_prefix;
  ...
  try {
    ...
    desc.add_options()("sampled_query_path",
                        po::value<std::string>(&sampled_query_path)->required(),
                        "Input sampled_query file in bin format");
  ...
  try {
    if (data_type == std::string("int8"))
      return diskann::build_disk_index_workload_aware<int8_t>(data_path.c_str(),
                                               sampled_query_path.c_str(),
                                               index_path_prefix.c_str(),
                                               params.c_str(), metric, use_opq);
    else if (data_type == std::string("uint8"))
      return diskann::build_disk_index_workload_aware<uint8_t>(
          data_path.c_str(), sampled_query_path.c_str(), index_path_prefix.c_str(), params.c_str(), metric,
          use_opq);
    else if (data_type == std::string("float"))
      return diskann::build_disk_index_workload_aware<float>(data_path.c_str(),
                                              sampled_query_path.c_str(),
                                              index_path_prefix.c_str(),
                                              params.c_str(), metric, use_opq);
    else {
