...
int search_disk_index(
  ...
  std::unique_ptr<diskann::PQFlashIndexWorkloadAware<T>> _pFlashIndex(
      new diskann::PQFlashIndexWorkloadAware<T>(reader, metric));
...
#pragma omp parallel for schedule(dynamic, 1)
    for (_s64 i = 0; i < (int64_t) warmup_num; i++) {
      _pFlashIndex->cached_beam_search_block_utilized(warmup + (i * warmup_aligned_dim), 1,
                                       warmup_L,
  ...
  for (uint32_t test_id = 0; test_id < Lvec.size(); test_id++) {
    ...
    if (beamwidth <= 0) {
      ...
      optimized_beamwidth =
        optimize_beamwidth_workload_aware(_pFlashIndex, warmup, warmup_num,
...
#pragma omp parallel for schedule(dynamic, 1)
    for (_s64 i = 0; i < (int64_t) query_num; i++) {
      _pFlashIndex->cached_beam_search_block_utilized(
