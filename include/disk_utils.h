...
namespace diskann {
  ...
  class PQFlashIndex;
  template<typename T>
  class PQFlashIndexWorkloadAware;
  ...
  template<typename T>
  DISKANN_DLLEXPORT int build_merged_vamana_index_query_aware(
      std::string base_file, std::string sampled_query_file, 
  ...
  template<typename T>
      DISKANN_DLLEXPORT uint32_t optimize_beamwidth_workload_aware(
          std::unique_ptr<diskann::PQFlashIndexWorkloadAware<T>> &_pFlashIndex, T *tuning_sample,
          _u64 tuning_sample_num, _u64 tuning_sample_aligned_dim, uint32_t L,
          uint32_t nthreads, uint32_t start_bw = 2);
  ...
  template<typename T>
  DISKANN_DLLEXPORT int build_disk_index_workload_aware(
  ...
  template<typename T>
  DISKANN_DLLEXPORT void create_disk_layout_workload_aware(
      const std::string &base_file, const std::string &mem_index_file,
